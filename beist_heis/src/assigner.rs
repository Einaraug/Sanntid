...
struct AssignerInput {
    ...
    hall_requests: [[bool; N_BUTTONS]; N_FLOORS],
...
fn build_input(wv: &WorldView) -> AssignerInput {
    let hall_requests = std::array::from_fn(|floor| [
        wv.order_table.hall[floor][0] == OrderState::Confirmed,
        wv.order_table.hall[floor][1] == OrderState::Confirmed,
    ]);

    let states = wv.elevator_map.elevator
        .iter()
        .filter(|(id, _)| wv.peer_availability.peer_availability.get(*id).copied().unwrap_or(false))
        .map(|(id, e)| (id.to_string(), ElevatorStateDto {
