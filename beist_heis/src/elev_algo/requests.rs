...
impl Elevator {
    pub fn requests_above(&self) -> bool {
        for floor in (self.floor + 1) as usize..N_FLOORS {
            for btn in 0..N_BUTTONS {
                if self.requests[floor][btn] {
                    return true;
                }
            }
        }
    ...
    pub fn requests_below(&self) -> bool {
        ...
        for floor in 0..self.floor as usize {
            for btn in 0..N_BUTTONS {
                if self.requests[floor][btn] {
                    return true;
                }
    ...
    pub fn clear_at_current_floor(&self) -> Self {
        ...
        elevator.requests[floor][Button::Cab.to_index()] = false;

        match elevator.dirn {
            Dirn::Up => {
                if !elevator.requests_above() && !elevator.requests[floor][Button::HallUp.to_index()] {
                    elevator.requests[floor][Button::HallDown.to_index()] = false;
                }
                elevator.requests[floor][Button::HallUp.to_index()] = false;
            }
            Dirn::Down => {
                if !elevator.requests_below() && !elevator.requests[floor][Button::HallDown.to_index()] {
                    elevator.requests[floor][Button::HallUp.to_index()] = false;
                }
                elevator.requests[floor][Button::HallDown.to_index()] = false;
            }
