...
void buf_push(struct BoundedBuffer* buf, int val){    
    ...
    sem_wait(&buf -> capacity); //Takes capacity and subtracts 1. If 0 wait
    ...
    sem_post(&buf->numElements); //Increments num elems
}

int buf_pop(struct BoundedBuffer* buf){
    // TODO: same, but different?
    sem_wait(&buf -> numElements); //Must be elem to pop
    
    pthread_mutex_lock(&buf -> mtx);
    int val = rb_pop(buf->buf);    
    pthread_mutex_unlock(&buf->mtx);

    sem_post(&buf->capacity); //Increments num elems
    return val;
}
